...
def normalize_text(text):
    ...
    replace_list = {
        ...
        "💄": " positive ", "💎": " positive ", "💩": " negative ","😕": " nagative ", "😱":  " nagative ", "😸": " positive ",
        ...
        '❤️': ' positive ', '🤬' : ' negative ', '🤩': ' positive ', '❌': '. ', '🤗': " positive ", '🧐': ' nagative ',
        '😵': ' nagative ', '😶': ' nagative ', '🙁': ' nagative ', '🥰' : ' positive ', '☀️': ' positive ', 
        ...
        'dep': u' đẹp ',u' xau ': u' xấu ','delicious': u' ngon ', u'hàg': u' hàng ', u'qủa': u' quả ', u'gd' : u'gia đình',
...
class RemovePunct(BaseEstimator, TransformerMixin):
    non_special_chars =  re.compile(pattern = "["
    ...
    def fit(self, x, y=None):
        return self
class Removetab(BaseEstimator, TransformerMixin):
    non_special_chars =  re.compile(pattern = "["
        u"\t"# emoticons
        u"\n"                            
                                    "]+", flags = re.UNICODE)
...
def preprocessing_batch(batch_text):
    train_text1 = []
    for t in batch_text:
        train_text1.append(normalize_text(t))
    train_text1 = RemoveConsecutiveSpaces().transform(x=train_text1)
    # train_text1 =  Lowercase().transform(x= train_text1)
    train_text1 =  RemovePunct().transform(x= train_text1)
    train_text1 =  Removetab().transform(x= train_text1)
    for t in train_text1:
      t = t.strip()
    
    return train_text1

def preprocessing_sample(text):
    train_text1 = []
    for t in [text]:
        train_text1.append(normalize_text(t))
    train_text1 = RemoveConsecutiveSpaces().transform(x=train_text1)
