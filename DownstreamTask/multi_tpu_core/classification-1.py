...
#     def __init__(self, bert, num_class, config):
...
#     sys.exit()
    ...
    if args.dataset=='FinancialPhraseBank':
        ...
        for i in range(len(df)):
            text, cl = ftfy.fix_text(df['Text'][i]), df['Class'][i]
            data.append([text, label2index[cl]])
    else:
        label2index = {'Negative': 0, 'Neutral': 1, 'Positive': 2}
        df = pd.read_csv(args.data_file[0], encoding='utf-8', index_col=False)
        data = []
        for i in range(len(df)):
            text, cl = df['text'][i], df['label'][i]
            data.append([text, label2index[cl]])
    ...
    if args.retrain_model:
        ...
        if '/' in args.tokenizer_name:
            model_name = args.tokenizer_name.split('/')[-1]
        result_file = '_'.join(['Evaluation', 'dataset', args.dataset, model_name, str(datetime.now()), 'task.txt'])
...
#                 config = AutoConfig.from_pretrained(args.config, from_tf=False, output_hidden_state=False)
                pt_bert = AutoModel.from_pretrained(args.tokenizer_name)
                bert = load_flax_checkpoint_in_pytorch_model(pt_bert, args.model_name_or_path)
                    
            else:
                bert = AutoModel.from_pretrained(args.tokenizer_name)
            ...
            if args.freeze_layer_count:
                ...
                if args.freeze_layer_count != -1:
                    ...
                    for layer in bert.bert.encoder.layer[:args.freeze_layer_count]:
                        for param in layer.parameters():
                            param.requires_grad = False
                            
            model = ClassificationBasedBert(bert, args.num_labels, config)
            """
            optimizer
            """
            # optimizer = AdamW(model.parameters(), lr=args.learning_rate)
